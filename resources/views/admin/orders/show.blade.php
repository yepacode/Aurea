    ...
    <div class="os-wrap max-w-5xl space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="space-y-6">
                ...
                {{-- Link de pago / seguimiento enviable al cliente --}}
                @if($order->tracking_token)
                @php
                    $trackUrl = route('order.track', $order->tracking_token);
                    $isPaid   = $order->payment_status === 'paid';

                    // Teléfono del cliente en formato E.164 (solo dígitos) para wa.me
                    $rawPhone = (string) ($order->customer->phone ?? '');
                    $phoneDigits = preg_replace('/\D+/', '', $rawPhone);
                    // Si tiene 10 dígitos y arranca con 3 (celular COL) asumimos Colombia
                    ...
                    $totalFmt  = '$' . number_format($order->total, 0, ',', '.') . ' COP';

                    $waMessage = $isPaid
                        ? "Hola {$firstName}, aquí está el seguimiento de tu pedido #{$order->id}: {$trackUrl}"
                        : "Hola {$firstName}, este es el link para pagar tu pedido #{$order->id}: {$trackUrl}\n\nTotal: {$totalFmt}";

                    $waHref = $phoneDigits
                        ? 'https://wa.me/' . $phoneDigits . '?text=' . rawurlencode($waMessage)
                        : 'https://wa.me/?text=' . rawurlencode($waMessage);
                @endphp
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    <h2 class="text-sm font-semibold text-gray-800 mb-1">
                        {{ $isPaid ? 'Link de seguimiento' : 'Link de pago para el cliente' }}
                    </h2>
                    <p class="text-xs text-gray-500 mb-3">
                        @if($isPaid)
                            El pedido ya está pagado. Puedes seguir compartiendo este link para el seguimiento del envío.
                        @else
                            Copia esta URL y compártela por WhatsApp o correo. El cliente verá el resumen y podrá pagar por ePayco o transferencia.
                        @endif
                    </p>
                    ...
                    <div class="flex flex-wrap gap-2">
                        {{-- Botón WhatsApp: si tenemos teléfono va directo al chat, si no, abre el selector de contacto --}}
                        <a href="{{ $waHref }}" target="_blank" rel="noopener"
                           class="inline-flex items-center gap-1.5 bg-green-600 hover:bg-green-700 text-white text-xs px-3 py-1.5 rounded font-medium transition-colors">
                            <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.019-.458.13-.606.134-.133.298-.347.446-.52.15-.174.199-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.095 3.2 5.076 4.487.71.306 1.263.489 1.694.626.712.226 1.36.194 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347zM12.05 21.785h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884zm8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                            </svg>
                            Enviar por WhatsApp
                            @if(!$phoneDigits)
                                <span class="opacity-75 font-normal">(elegir contacto)</span>
                            @endif
                        </a>

                        {{-- Correo: mailto simple, no necesita backend --}}
                        @php
                            $mailSubject = $isPaid
                                ? "Seguimiento de tu pedido #{$order->id} - Belleza Áurea"
                                : "Link para pagar tu pedido #{$order->id} - Belleza Áurea";
                            $mailBody = "Hola {$firstName},\n\n"
                                . ($isPaid
                                    ? "Aquí está el link para el seguimiento de tu pedido:\n{$trackUrl}\n\n"
                                    : "Aquí está el link para completar el pago de tu pedido:\n{$trackUrl}\n\nTotal a pagar: {$totalFmt}\n\n")
                                . "¡Gracias por elegir Belleza Áurea!";
                            $mailHref = 'mailto:' . rawurlencode($order->customer->email)
                                . '?subject=' . rawurlencode($mailSubject)
                                . '&body=' . rawurlencode($mailBody);
                        @endphp
                        <a href="{{ $mailHref }}"
                           class="inline-flex items-center gap-1.5 bg-gray-100 hover:bg-gray-200 text-gray-700 text-xs px-3 py-1.5 rounded font-medium transition-colors">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
